...
// Step 3: output information about the segment in the required format
	...
	printf("----		----		----		-----	----	-----\n");
	printf("%d 		%d		", segment_id, IPC_PRIVATE);
	...
	//Finish calls to structure
	printf("	%d	%ld 	%ld\n" , shmbuffer.shm_cpid, shmbuffer.shm_segsz, shmbuffer.shm_nattch);
...
//         the message it received from the child process
	...
	if (pid == 0)
		printf("Message returned from Child: Hello!\n");
	else if (pid > 0)
		wait(NULL);
